...
const NotificationDialog = ({ isOpen, onClose }) => {
  const { notifications, markNotificationAsRead } = useAuth(); // Fixed: Added parentheses to call the hook
  const [processingIds, setProcessingIds] = useState(new Set());

  useEffect(() => {
    // No additional fetch needed here as notifications are managed by AuthContext
  }, [notifications]);

  if (!isOpen) return null;

  // Ensure notifications is always an array to prevent undefined errors
