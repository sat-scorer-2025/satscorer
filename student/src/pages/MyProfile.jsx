...
const MyProfile = () => {
  const { fetchProtected, authLoading, authError, user: authUser, refreshUserData } = useContext(AuthContext);
  const { courses, fetchCourses, isLoading: coursesLoading, error: coursesError } = useStudentContext();
  const [isEditing, setIsEditing] = useState(false);
  const [user, setUser] = useState(null);
  const [formData, setFormData] = useState({
  ...
  const [formError, setFormError] = useState('');
  const [successMessage, setSuccessMessage] = useState('');
  const [hasFetched, setHasFetched] = useState(false); // Track if profile data has been fetched

  const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000';

  const fetchProfileData = async () => {
    if (!authUser || hasFetched) return; // Prevent redundant fetches
    setFormError('');
    ...
    try {
      ...
      if (!user || JSON.stringify(user) !== JSON.stringify(userData.user)) {
        setUser(userData.user);
        setFormData({
          name: userData.user.name || '',
          email: userData.user.email || '',
      ...
      // Only fetch courses if not already fetched
      if (!courses.length) {
        await fetchCourses();
      }
  ...
  useEffect(() => {
    if (authUser && !authLoading && !hasFetched) {
      fetchProfileData();
    }
  }, [authUser, authLoading, hasFetched]); // Include hasFetched to prevent re-fetching
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formData.exam) formDataToSend.append('exam', formData.exam);
      if (formData.university) formDataToSend.append('university', formData.university);
      if (formData.password) formDataToSend.append('password', formData.password);
      if (profilePicFile) formDataToSend.append('profilePhoto', profilePicFile);
      ...
      setProfilePicFile(null);
      setSuccessMessage('Profile updated successfully!');
      await refreshUserData(); // Refresh AuthContext user data
      setHasFetched(false); // Allow re-fetch after update
      setTimeout(() => setSuccessMessage(''), 3000);
    } catch (err) {
      setFormError(err.message || 'Failed to update profile');
      console.error('Profile update client error:', err);
    }
  };
